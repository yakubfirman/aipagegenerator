...
import {
    ...
    Users, Tag, Star, DollarSign, ArrowLeft, CheckCircle2,
} from 'lucide-react';
...
const InputField = ({ label, icon: Icon, required, error, max, value, children }) => (
    <div>
    ...
    return (
        <div className="border-b border-slate-800 px-6 py-3 bg-slate-900/50">
            ...
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-800">
                <div className={`h-full rounded-full transition-all duration-500 ${pct === 100 ? 'bg-emerald-500' : 'bg-gradient-to-r from-violet-500 to-indigo-500'}`} style={{ width: `${pct}%` }} />
            </div>
        </div>
    );
}
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center gap-3">
                    <Link href={route('sales-pages.show', page.id)} className="flex shrink-0 items-center justify-center rounded-sm border border-slate-700 bg-slate-800 p-2 text-slate-400 transition hover:border-slate-600 hover:text-white">
                ...
                <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-6 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
                    ...
                    <div className="rounded-sm border border-slate-800 bg-slate-900">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5 p-5 sm:p-6">
                            <InputField label="Nama Produk / Layanan" icon={Package} required error={errors.product_name} max={100} value={data.product_name}>
                                <input type="text" value={data.product_name} onChange={(e) => setData('product_name', e.target.value)} className={inputClass} maxLength={100} />
                            </InputField>
                            <InputField label="Deskripsi Produk" icon={FileText} required error={errors.description} max={600} value={data.description}>
                                <textarea rows={4} value={data.description} onChange={(e) => setData('description', e.target.value)} className={inputClass} maxLength={600} />
                            </InputField>
                            <InputField label="Fitur Utama" icon={Star} required error={errors.features} max={400} value={data.features}>
                                <textarea rows={3} value={data.features} onChange={(e) => setData('features', e.target.value)} className={inputClass} placeholder="Pisahkan dengan koma" maxLength={400} />
                            </InputField>
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <InputField label="Target Audiens" icon={Users} required error={errors.target_audience} max={100} value={data.target_audience}>
                                    <input type="text" value={data.target_audience} onChange={(e) => setData('target_audience', e.target.value)} className={inputClass} maxLength={100} />
                                </InputField>
                                <InputField label="Harga" icon={DollarSign} required error={errors.price} max={50} value={data.price}>
                                    <input type="text" value={data.price} onChange={(e) => setData('price', e.target.value)} className={inputClass} maxLength={50} />
                                </InputField>
                            </div>
                            <InputField label="Keunggulan Unik (USP)" icon={Tag} error={errors.unique_selling_points} max={200} value={data.unique_selling_points}>
                                <textarea rows={2} value={data.unique_selling_points} onChange={(e) => setData('unique_selling_points', e.target.value)} className={inputClass} maxLength={200} />
                            </InputField>
                            <div className="flex flex-col-reverse gap-3 border-t border-slate-800 pt-5 sm:flex-row sm:justify-between">
                                <Link href={route('sales-pages.show', page.id)} className="flex items-center justify-center rounded-sm border border-slate-700 px-5 py-2.5 text-sm font-medium text-slate-400 transition hover:border-slate-600 hover:text-white">
                                    Batal
                                </Link>
